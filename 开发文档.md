...
- Prompt优化（中优先级）
  - 增强中文示例与说明，明确必须输出 `date_from`/`date_to` 的条件与格式
  - 加强结构化输出约束（固定字段名与类型，避免缺失或命名漂移）
- 小型LLM补齐（中优先级）
  - 当主路径未返回时间时，追加一次窄域 LLM 调用，仅提取 `date_from`/`date_to`
  - 控制模型温度与上下文，降低误差与冗余
- 时间归一化模块（高优先级但后置）
  - 引入专用时间解析器或定制中文相对时间语法，统一处理“近N天/近N周/最近半年/上季度/去年/前年/截至今天/至今”等表达
  - 归一化输出为 ISO8601 字符串，面向 Agent 层解析
- 降级解析扩展（低成本持续项）
  - 扩充字典与正则覆盖更多表达与边界（含“个”的数词用法）
- 可观测性与开关（运维保障）
  - 完善日志指标与监控，增加开关（如 `TIME_FALLBACK_ENABLED`）以便快速回滚/禁用
- 验收标准
  - LLM 主路径时间命中率提升、降级触发率下降、端到端查询结果正确率提升
